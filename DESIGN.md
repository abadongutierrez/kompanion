...
Three exist: `engineer` (implement-task skill, planner subagent), `qa` (verify-task skill, test-planner subagent), `product_manager` (refine-task skill, breakdown subagent). An Agent resolves its harness through its own `harness_path` column, read by `resolveHarnessDir()` in `server-kotlin/src/main/kotlin/com/kompanion/server/service/ClaudeHarnessService.kt` — no fallback, no discipline→directory mapping table. That column stores a path **relative to `WORKSPACE_ROOT`** (`harnesses/engineer`) for anything under `workspace/`, so a database dump stays valid on another machine; an absolute path is still accepted and stored verbatim for a harness living elsewhere on disk. `resolveHarnessPath()` handles both forms and `toStoredHarnessPath()` does the relativizing on the way in (see V16). (This started as a fixed `HARNESS_DIR_BY_DISCIPLINE` constant; the column replaced it once harnesses moved to the shared repo-root `workspace/`.)
