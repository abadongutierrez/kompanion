...
@Service
class WorkspaceEnforcementService(private val objectMapper: ObjectMapper) {

    // Mirrors ClaudeHarnessService's own cwd-relative resolution.
    private val enforcementScriptSrc = File(File(".").canonicalFile, "hooks/enforce-workspace.py")
    ...
    fun installCwdEnforcement(cwdDir: File, taskWorkspaceDir: File, manifest: WorkspaceManifest) {
        ...
        hooksDir.mkdirs()
        enforcementScriptSrc.copyTo(File(hooksDir, "enforce-workspace.py"), overwrite = true)
